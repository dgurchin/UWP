...
namespace Inventory.ViewModels
{
    ...
    public class AppLogDetailsViewModel : GenericDetailsViewModel<AppLogModel>
    {
        ...
        override public string Title => "Журнал активности";
        ...
        protected override async Task<bool> DeleteItemAsync(AppLogModel model)
        {
            try
            {
                StartStatusMessage("Удаление записей...");
                await Task.Delay(100);
                await LogService.DeleteLogAsync(model);
                EndStatusMessage("Записи удалены");
                return true;
            }
            catch (Exception ex)
            {
                StatusError($"Ошибка удаления записи: {ex.Message}");
        ...
        {
            return await DialogService.ShowAsync("Подтвердите удаление", "Вы уверены,что хотите удалить текущую запись журнала?", "Ok", "Отменить");
        }
        ...
        private async Task OnItemDeletedExternally()
        {
            await ContextService.RunAsync(() =>
            {
                CancelEdit();
                IsEnabled = false;
                StatusMessage("Предупреждение: Эта запись журнала была удалена извне");
            });
        }
    }
}
