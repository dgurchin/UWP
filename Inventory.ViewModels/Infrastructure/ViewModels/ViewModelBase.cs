...
namespace Inventory.ViewModels
{
    public class ViewModelBase : ObservableObject
    {
        ...
        {
            _stopwatch.Stop();
            StatusMessage($"{message} ({_stopwatch.Elapsed.TotalSeconds:#0.000} секунд)");            
        }

        public void StatusReady()
        {
            MessageService.Send(this, Messages.StatusMessage, Messages.StatusMessageReady);
        }
        public void StatusMessage(string message)
        {
            Microsoft.AppCenter.Analytics.Analytics.TrackEvent(message);
            MessageService.Send(this, Messages.StatusMessage, message);
        }
        public void StatusError(string message)
        {
            Microsoft.AppCenter.Analytics.Analytics.TrackEvent(message);
            MessageService.Send(this, Messages.StatusError, message);
        }

        public void EnableThisView(string message = null)
        {
            message = message ?? Messages.StatusMessageReady;
            MessageService.Send(this, Messages.EnableThisView, message);
        }
        public void DisableThisView(string message)
        {
            MessageService.Send(this, Messages.DisableThisView, message);
        }

        public void EnableOtherViews(string message = null)
        {
            message = message ?? Messages.StatusMessageReady;
            MessageService.Send(this, Messages.EnableOtherViews, message);
        }
        public void DisableOtherViews(string message)
        {
            MessageService.Send(this, Messages.DisableOtherViews, message);
        }

        public void EnableAllViews(string message = null)
        {
            message = message ?? Messages.StatusMessageReady;
            MessageService.Send(this, Messages.EnableAllViews, message);
        }
        public void DisableAllViews(string message)
        {
            MessageService.Send(this, Messages.DisableAllViews, message);
        }
    }
}
