...
{
    public sealed partial class DataGrid : UserControl, INotifyPropertyChanging
    {
        ...
        public static readonly DependencyProperty DefaultCommandsProperty = DependencyProperty.Register(nameof(DefaultCommands), typeof(string), typeof(DataGrid), new PropertyMetadata("new,select,refresh,search"));
        #endregion
        ...
        public string DataUnavailableMessage => ItemsSource == null ? "Загружается..." : "Данных не найдено.";
                        ...
                        if (ItemsSource is ISelectionInfo selectionInfo)
                        {
                            if (selectionInfo.IsSelected(e.NewStartingIndex))
                            {
                                if (!ignoreSelectedItemValue)
                                {
                                    ...
                                    System.Diagnostics.Debug.WriteLine("Вибрано {0}", SelectedItem);
                                }
                            }
                        }
                    }
        ...
        private void Gridview_KeyUp(object sender, Windows.UI.Xaml.Input.KeyRoutedEventArgs e)
        {
            keyDownCount = 0;
