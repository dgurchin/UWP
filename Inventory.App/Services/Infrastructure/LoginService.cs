...
namespace Inventory.Services
{
    public class LoginService : ILoginService
    {
        ...
        public async Task TrySetupWindowsHelloAsync(string userName)
        {
            ...
            {
                if (await DialogService.ShowAsync("Windows Hello", "Ваше устройство поддерживает Windows Hello, и вы можете использовать его для аутентификации в приложении.\r\n\r\nВы хотите включить Windows Hello для вашего следующего входа в систему с этим пользователем?", "Ok", "Может быть позже"))
                {
        ...
        private async Task<IBuffer> CreatePassportKeyCredentialAsync(string userName)
        {
            ...
            {
                await DialogService.ShowAsync("Windows Hello", "Для продолжения Windows Hello необходимо настроить в настройках Windows (Учетные записи -> Параметры входа)");
            }
            else if (keyCreationResult.Status == KeyCredentialStatus.UnknownError)
            {
                await DialogService.ShowAsync("Windows Hello Ошибка", "Ключ доступа не может быть создан. Пожалуйста, попробуйте еще раз.");
            }
